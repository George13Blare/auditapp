...
def run_analysis(
    ...
    if config.group_by == "study":
        ...
        study_items: List[Tuple[str, Union[str, List[str]]]] = list(studies_dict.items())
    else:
    ...
    progress_iter = study_items
    
    if config.show_progress and HAS_TQDM:
        progress_iter = tqdm(study_items, desc="Обработка исследований")
        ...
        for key, value in study_items:
            if config.group_by == "study":
                ...
                future = executor.submit(
                    process_study_uid, uid, files, config, debug
                )
            else:
                study_path = value if isinstance(value, str) else str(value)
                future = executor.submit(
                    process_study_dir, study_path, config, debug
                )
            futures[future] = key
    ...
    report.patient_study_counts = {
        pid: len(studies) for pid, studies in patient_study_map.items()
    }
...
def print_summary(report: AnalysisReport) -> None:
    ...
    if report.patient_study_counts:
        ...
        top_patients = sorted(
            report.patient_study_counts.items(),
            key=lambda x: x[1],
            reverse=True
        )[:5]
